...
public class Mariadb4jStartMojo extends AbstractMojo {
  ...
  public static final String DB_URL = "mariadb4j.url";
  /** Maven project. */
  ...
  @Parameter private String databaseVersion;

  @Parameter private String libDir;

  @Parameter private File tempDir;

  @Parameter private String os;

  @Parameter(
    property = Mariadb4jFindFreePortMojo.PROPNAME_PORT,
    defaultValue = UNSET_PORT_VALUE_STR
  )
  private int port;
  ...
  @Parameter private String scriptCharset;

  @Parameter private String createDatabase;
  ...
  protected void runScripts(DB db, @Nullable String dbName)
      throws ManagedProcessException, IOException, MojoExecutionException {
  ...
  @SuppressWarnings("unchecked")
  private File getOrCreateSupportDirectory(
      @Nullable String pathname, SupportDir supportDir, Map pluginContext) throws IOException {
    if (pathname == null) {
      ...
      Path dir =
          java.nio.file.Files.createTempDirectory(tempParent.toPath(), supportDir.toPrefix());
  ...
  protected DBConfiguration buildDbConfiguration(Map pluginContext) throws IOException {
    ...
    b.setBaseDir(
        getOrCreateSupportDirectory(baseDir, SupportDir.base, pluginContext).getAbsolutePath());
    b.setDataDir(
        getOrCreateSupportDirectory(dataDir, SupportDir.data, pluginContext).getAbsolutePath());
    if (databaseVersion != null) {
      b.setDatabaseVersion(databaseVersion);
    }
    b.setLibDir(
        getOrCreateSupportDirectory(libDir, SupportDir.lib, pluginContext).getAbsolutePath());
    if (os != null) {
      b.setOS(os);
    }
    checkState(
        port != UNSET_PORT_VALUE,
        "port must be set to value >= 0; "
            ...
            + "actual port value on this run is %s",
        port);
    b.setPort(port);
    if (socket != null) {
      b.setSocket(socket);
    ...
    if (createDatabase != null && !createDatabase.isEmpty()) {
      String url = b.getURL(createDatabase);
      System.setProperty(DB_URL, url);
        getLog().info("Set url for db " + DB_URL + " = " + url);
    }
    return b.build();
  }
